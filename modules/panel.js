 ...
 * into the string, so a path is never re-parsed as shell syntax.
 *
 ...
 * `application/x-remmina`, which on a flatpak install is Remmina's own
 * `-c` connect entry with `--file-forwarding`. That means the portal maps the
 * path into the sandbox, an already-running Remmina picks the request up, and
 * the identical code works for a distribution package.
 *
